          ...
          <div key={p.id} className="admin-card">
            ...
            {editing === p.id && (
              <div className="border-t pt-4 mt-4 grid md:grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-medium text-gray-500 mb-1">Nav Order</label><input type="number" inputMode="numeric" value={p.nav_order} onChange={e => setPagesList(pagesList.map(x => x.id === p.id ? {...x, nav_order: parseInt(e.target.value) || 0} : x))} className="admin-input" /></div>
